...
local state =
    { bufnr = nil, winid = nil, expanded = {}, help = false, tags = {} }
...
local function render()
    ...
    -- Header
    put(
        (" vim.pack  %d plugins  (%d loaded, %d not loaded)"):format(
            #plugins,
            #loaded,
            #unloaded
        ),
    ...
    local bar =
        "  [U] Update All   [u] Update   [D] Delete   [L] Log   [?] Help"
    put(bar)
    ...
    if state.help then
        ...
        put(
            "    D        Delete plugin under cursor (inactive only)",
            "PackUiHelp"
        )
    ...
    if #unloaded > 0 then
        put("")
        put(
            (" Not Loaded (%d)  — safe to delete with D"):format(#unloaded),
            "PackUiSection"
        )
...
local function open()
    ...
    vim.keymap.set("n", "D", function()
        ...
        if pdata[1] and pdata[1].active then
            vim.notify(
                ("vim.pack: %s is active — remove it from your config first"):format(
                    name
                ),
                vim.log.levels.WARN
            )
        ...
        if vim.fn.confirm(("Delete %s?"):format(name), "&Yes\n&No", 2) == 1 then
            ...
            vim.notify(
                ok and ("vim.pack: deleted %s"):format(name)
                    or "vim.pack: " .. tostring(err),
    ...
    api.nvim_create_autocmd("WinClosed", {
        ...
        callback = function(ev)
            if vim._tointeger(ev.match) == state.winid then
                state.winid, state.bufnr, state.expanded, state.help =
                    nil, nil, {}, false
            end
        end,
    })
