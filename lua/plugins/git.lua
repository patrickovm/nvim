...
vim.keymap.set(
    ...
    "<leader>gd",
    "<Cmd>VDiffCompare<CR>",
    { desc = "Git: working tree (all files)" }
)
vim.keymap.set(
    "n",
    "<leader>gD",
    "<Cmd>VDiffCompare --staged<CR>",
    { desc = "Git: staged (all files)" }
)
vim.keymap.set(
    "n",
    "<leader>gV",
...
vim.keymap.set(
    ...
    "<leader>gf",
    "<Cmd>VDiff<CR>",
    { desc = "Git: diff file vs HEAD" }
)
vim.keymap.set("n", "<leader>gF", function()
    vim.ui.input(
        { prompt = "Diff file with (ref, empty=HEAD): " },
        function(ref)
        ...
        vim.ui.input({ prompt = "Second file: " }, function(file2)
            if file2 and file2:match("%S") then
                ...
                if vim.fn.executable("difft") == 1 then
                    ...
                    vim.cmd(
                        ("tabnew | e %s | diffthis | vsplit %s | diffthis"):format(
                            abs1,
                            abs2
                        )
                    )
